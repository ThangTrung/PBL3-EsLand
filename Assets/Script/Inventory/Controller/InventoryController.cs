...
namespace Script.Inventory.Controller
{
    public class InventoryController : MonoBehaviour
    {
        ...
        public bool AddItem(Item item, int amount = 1)
        {
            ...
            if (item.MaxStack > 1)
            {
                foreach (var slot in _slots.Where(slot => slot.Item == item))
                {
            ...
            while (rem > 0)
            {
                if (IsFull) { RaiseChanged(); return false; }
        ...
        private bool HasItem(Item item, int amount = 1) => 
            _slots.Where(s => s.Item == item).Sum(s => s.Amount) >= amount;

        public int CountItem(Item item) => 
            _slots.Where(s => s.Item == item).Sum(s => s.Amount);

        public InventorySlot GetSlot(Item item) => _slots.Find(s => s.Item == item);
        public InventorySlot GetSlotAt(int index) => (index >= 0 && index < _slots.Count) ? _slots[index] : null;
